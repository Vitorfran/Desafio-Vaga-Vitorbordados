...
export default function Dashboard({ mudarTela }) {
  const [dados, setDados] = useState({ nome: 'Candidato', perfil: null, desafio: null, submissao: null, status: 'AGUARDANDO_ENVIO', segundaEtapa: null, carregando: true })
  const [segundaMensagem, setSegundaMensagem] = useState('')
  const [salvandoSegunda, setSalvandoSegunda] = useState(false)
  useEffect(() => {
    let ativo = true
    async function carregar() {
      ...
      const [perfil, desafio, submissao, status, segundaEtapa] = await Promise.all([
        ...
        supabase.from('candidate_second_stage_responses').select('*').eq('candidate_id', sessao.user.id).maybeSingle(),
      ])
      if (ativo) setDados({ nome: sessao.user.user_metadata?.nome || sessao.user.email?.split('@')[0] || 'Candidato', perfil: perfil.data, desafio: desafio.data, submissao: submissao.data, status: status.data?.status || submissao.data?.status || 'AGUARDANDO_ENVIO', segundaEtapa: segundaEtapa.data, carregando: false })
    }
    carregar()
    return () => { ativo = false }
  }, [])

  async function salvarSegundaEtapa(event) {
    event.preventDefault()
    setSalvandoSegunda(true)
    setSegundaMensagem('')
    const formulario = new FormData(event.currentTarget)
    const { data: sessao } = await supabase.auth.getUser()
    if (!sessao.user) { setSegundaMensagem('Sua sessão expirou. Faça login novamente.'); setSalvandoSegunda(false); return }
    const resposta = {
      candidate_id: sessao.user.id,
      embroidery_experience: formulario.get('embroidery_experience'),
      availability: formulario.get('availability'),
      average_production: formulario.get('average_production'),
      can_issue_invoice: formulario.get('can_issue_invoice') === 'true',
      updated_at: new Date().toISOString(),
    }
    const { data, error } = await supabase.from('candidate_second_stage_responses').upsert(resposta, { onConflict: 'candidate_id' }).select().single()
    if (error) { setSegundaMensagem(`Não foi possível salvar: ${error.message}`); setSalvandoSegunda(false); return }
    setDados(atual => ({ ...atual, segundaEtapa: data }))
    setSegundaMensagem('Respostas salvas com sucesso.')
    setSalvandoSegunda(false)
  }
