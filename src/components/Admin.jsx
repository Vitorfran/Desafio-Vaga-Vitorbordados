...
export default function Admin() {
  ...
  return <>
    ...
    <section className="card tabela-card"><div className="tabela-topo"><div><p className="eyebrow">CANDIDATOS</p><h3>Candidatos cadastrados <span>{candidatos.length}</span></h3></div></div>{carregando ? <div className="estado-vazio"><p>Aguarde...</p></div> : candidatos.length === 0 ? <div className="estado-vazio"><UsersRound size={30} /><h2>Nenhum candidato cadastrado</h2><p>Os candidatos reais aparecerão aqui após criarem uma conta.</p></div> : <div className="tabela-scroll"><table><thead><tr><th>CANDIDATO</th><th>WILCOM</th><th>STATUS</th><th>NOTA</th><th>ATUALIZAÇÃO</th></tr></thead><tbody>{candidatos.map(item => <tr key={item.id} onClick={() => abrirDetalhe(item.id)}><td><div className="candidato-celula"><div className="avatar avatar-laranja">{(item.nome || 'C').slice(0, 2).toUpperCase()}</div><div><strong>{item.nome}</strong><small>{item.email}</small></div></div></td><td>{item.wilcom || '—'}<small>{item.nivel || ''}</small></td><td>{item.status}</td><td>{item.nota ?? '—'}</td><td>{dataExibicao(item.atualizado_em)}</td></tr>)}</tbody></table></div>}</section>
