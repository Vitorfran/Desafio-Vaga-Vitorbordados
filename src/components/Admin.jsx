...
export default function Admin() {
  ...
  const candidatosFiltrados = candidatos.filter(item => filtroCandidatos === 'TODOS' || (filtroCandidatos === 'ENTREGAS' ? item.tem_entrega : item.status === filtroCandidatos))
  const totalEntregas = candidatos.filter(item => item.tem_entrega || item.status !== 'AGUARDANDO_ENVIO').length
  ...
  return <>
    ...
    <div className="admin-stats"><div><span className="icone-box azul-claro"><UsersRound size={18} /></span><strong>{candidatos.length}</strong><small>Total de candidatos</small></div><div className="admin-stat-clicavel" onClick={() => setFiltroCandidatos('ENTREGAS')} role="button" tabIndex={0}><span className="icone-box amarelo"><FileText size={18} /></span><strong>{totalEntregas}</strong><small>Entregas recebidas</small></div><div><span className="icone-box roxo-claro"><Clock3 size={18} /></span><strong>{candidatos.filter(item => item.status === 'EM_ANALISE' || item.status === 'ENVIADO').length}</strong><small>Em avaliação</small></div><div><span className="icone-box verde-claro"><Check size={18} /></span><strong>{candidatos.filter(item => item.status === 'APROVADO').length}</strong><small>Aprovados</small></div></div>
    <section className="card tabela-card"><div className="tabela-topo"><div><p className="eyebrow">CANDIDATOS</p><h3>Candidatos cadastrados <span>{candidatos.length}</span></h3></div></div><div className="admin-abas" role="tablist" aria-label="Filtrar candidatos">{[['TODOS', 'Todos'], ['ENTREGAS', 'Entregas recebidas'], ['APROVADO', 'Aprovados'], ['EM_ANALISE', 'Em análise'], ['DESCLASSIFICADO', 'Desclassificados']].map(([valor, rotulo]) => <button key={valor} className={filtroCandidatos === valor ? 'ativa' : ''} onClick={() => setFiltroCandidatos(valor)} role="tab" aria-selected={filtroCandidatos === valor}>{rotulo}<span>{valor === 'TODOS' ? candidatos.length : valor === 'ENTREGAS' ? totalEntregas : candidatos.filter(item => item.status === valor).length}</span></button>)}</div>{carregando ? <div className="estado-vazio"><p>Aguarde...</p></div> : candidatosFiltrados.length === 0 ? <div className="estado-vazio"><UsersRound size={30} /><h2>Nenhum candidato nesta aba</h2><p>Os candidatos aparecerão aqui quando tiverem este status.</p></div> : <div className="tabela-scroll"><table><thead><tr><th>CANDIDATO</th><th>WILCOM</th><th>DESAFIO</th><th>STATUS</th><th>NOTA</th><th>ATUALIZAÇÃO</th></tr></thead><tbody>{candidatosFiltrados.map(item => <tr key={item.id} onClick={() => abrirDetalhe(item.id)}><td><div className="candidato-celula"><div className="avatar avatar-laranja">{(item.nome || 'C').slice(0, 2).toUpperCase()}</div><div><strong>{item.nome}</strong><small>{item.email}</small></div></div></td><td>{item.wilcom || '—'}<small>{item.nivel || ''}</small></td><td><strong>{item.desafio_titulo || '—'}</strong><small>{item.tem_entrega ? 'Entrega registrada' : 'Sem entrega'}</small></td><td>{item.status}</td><td>{item.nota ?? '—'}</td><td>{dataExibicao(item.atualizado_em)}</td></tr>)}</tbody></table></div>}</section>
